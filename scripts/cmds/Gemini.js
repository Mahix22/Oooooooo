...
module.exports = {
  ...
  onStart: async function({
    ...
    try {
      ...
      if ("message_reply" === e.type && ["photo", "sticker"].includes(e.messageReply.attachments?.[0]?.type)) {
        ...
        const o = r.join(" "),
              c = await a.get(`${global.api.samirApi}/Gemini?text=${encodeURIComponent(o)}&uid=${i}`);
      ...
      const d = `${global.api.samirApi}/telegraph?url=${encodeURIComponent(s)}&senderId=${i}`,
            p = await a.get(d),
            u = p.data.result.link,
            o = r.join(" "),
            f = `${global.api.samirApi}/gemini-pro?text=${encodeURIComponent(o)}&url=${encodeURIComponent(u)}`;
      m.reply({
  ...
  onReply: async function({
    ...
    try {
      ...
      const i = n.join(" "),
            d = await a.get(`${global.api.samirApi}/Gemini?text=${encodeURIComponent(i)}&uid=${e.senderID}`);
